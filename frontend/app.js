...
function openAddItemModal() {
  ...
  modal.innerHTML = `<div class="modal-food add-item-modal"><div class="modal-food-info"><button class="close" type="button" id="close-add-item">×</button><p class="eyebrow">Kitchen catalog</p><h2>Add a new dish</h2><p>Give customers something delicious to discover.</p><form id="add-item-form" class="add-item-form"><div class="form-grid"><label>Dish name<input id="new-item-name" required placeholder="Truffle mushroom pasta"></label><label>Category<select id="new-item-category"><option>Starters</option><option>Main Course</option><option>Momos</option><option>Pizza</option><option>Burgers</option><option>Indian</option><option>Desserts</option><option>Drinks</option></select></label><label>Price<input id="new-item-price" type="number" min="1" step="0.01" required placeholder="299"></label><label>Image URL<input id="new-item-image" type="url" placeholder="https://..."></label><label class="full-field">Description<textarea id="new-item-description" rows="3" required placeholder="A short description of the dish"></textarea></label><label class="full-field">Ingredients<input id="new-item-ingredients" placeholder="Mushroom, cream, herbs"></label><label class="check-field"><input id="new-item-veg" type="checkbox"> Vegetarian</label><label class="check-field"><input id="new-item-available" type="checkbox" checked> Available now</label></div><button class="primary" type="submit">Add dish <span>→</span></button></form></div></div>`;
  modalBackdrop.classList.add('open');
  $('#close-add-item').onclick = () => modalBackdrop.classList.remove('open');
  $('#add-item-form').onsubmit = async event => {
    event.preventDefault();
    const payload = { name: $('#new-item-name').value.trim(), category: $('#new-item-category').value, description: $('#new-item-description').value.trim(), ingredients: $('#new-item-ingredients').value.split(',').map(item => item.trim()).filter(Boolean), price: Number($('#new-item-price').value), image: $('#new-item-image').value.trim() || 'https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&w=900&q=80', isVegetarian: $('#new-item-veg').checked, available: $('#new-item-available').checked, rating: 4.5 };
    try {
      const item = await api('/menu', { method: 'POST', body: JSON.stringify(payload) });
      ...
      toast(error.message || 'Could not add the dish');
    }
  };
}
async function track(event) { event.preventDefault(); try { const order = await api(`/orders/public/${$('#track-id').value.trim()}`); const statuses = ['Pending','Confirmed','Preparing','Ready','Completed']; $('#track-result').innerHTML = `<div style="display:grid;gap:6px"><strong>${order.orderId}</strong><small style="opacity:.8">${order.customerName || 'Customer'}</small><div>${statuses.map(status => `<span style="color:${statuses.indexOf(status) <= statuses.indexOf(order.status) ? '#f4bd55' : '#ffffff55'}">● ${status}</span>`).join(' &nbsp; ')}</div></div>`; } catch (error) { $('#track-result').textContent = error.message; } }
...
function togglePaymentMethod() {
  ...
  const upiPanel = getElement('#upi-payment');
  if (!upiPanel) return;
  upiPanel.classList.toggle('hidden', paymentMethod !== 'UPI');
}
function openGpay() {
  const amount = state.cart.reduce((sum, item) => sum + Number(item.price || 0) * Number(item.quantity || 0), 0);
  const total = Number((amount * 1.05 + 20).toFixed(2));
  const upiLink = `upi://pay?pa=foodiehub@okaxis&pn=FoodieHub&am=${total}&cu=INR`;
  try {
    window.location.href = upiLink;
    toast('Opening GPay to complete payment');
  } catch (error) {
    toast('Could not open GPay on this device');
  }
}
async function placeOrder(event) { event.preventDefault(); const type = $('#order-type').value; const payload = { customerName: $('#customer-name').value.trim(), mobile: $('#customer-mobile').value.trim(), email: $('#customer-email').value.trim(), tableNumber: $('#table-number').value.trim(), orderType: type, paymentMethod: $('#payment-method').value, specialInstructions: $('#instructions').value.trim(), items: state.cart }; if (type === 'Dine In' && !payload.tableNumber) return toast('Table number is required for dine-in'); if (!payload.paymentMethod) return toast('Select a payment method'); try { const order = await api('/orders', { method: 'POST', body: JSON.stringify(payload) }); state.cart = []; saveCart(); $('#checkout-modal').classList.remove('open'); $('#cart-drawer').classList.remove('open'); $('#track-id').value = order.orderId; $('#track-result').innerHTML = `<strong>Order placed successfully.</strong> Payment: ${order.paymentMethod}. Your ID is ${order.orderId}.`; $('#checkout-form').reset(); togglePaymentMethod(); if ($('#menu')) document.getElementById('menu').scrollIntoView({ behavior: 'smooth', block: 'start' }); location.hash = 'menu'; toast('Order placed successfully'); } catch (error) { toast(error.message); } }

function goToCustomerHome() {
  const customerView = $('#customer-view');
  const ownerViewEl = $('#owner-view');
  const dashboard = $('#dashboard');
  const ownerLogin = $('#owner-login');
  if (customerView) customerView.classList.remove('hidden');
  if (ownerViewEl) ownerViewEl.classList.add('hidden');
  if (dashboard) dashboard.classList.add('hidden');
  if (ownerLogin) ownerLogin.classList.remove('hidden');
  setAuthMode('login');
...
document.addEventListener('DOMContentLoaded', () => { loadMenu(); renderCart(); $('#search').oninput = renderMenu; $('#category').onchange = renderMenu; $('#diet').onchange = renderMenu; $('#sort').onchange = renderMenu; $('#category-pills').onclick = event => { const category = event.target.dataset.category; if (!category) return; $('#category').value = category; document.querySelectorAll('#category-pills button').forEach(button => button.classList.toggle('active', button === event.target)); renderMenu(); }; $('#menu-grid').ondblclick = event => { const card = event.target.closest('.food-card'); if (card) openFood(card.querySelector('[data-add]').dataset.add); }; $('#open-cart').onclick = () => $('#cart-drawer').classList.add('open'); $('#close-cart').onclick = () => $('#cart-drawer').classList.remove('open'); $('#checkout').onclick = showCheckout; $('#close-checkout').onclick = () => $('#checkout-modal').classList.remove('open'); $('#checkout-form').onsubmit = placeOrder; $('#payment-method').onchange = togglePaymentMethod; $('#gpay-link').onclick = openGpay; $('#track-form').onsubmit = track; $('#back-home').onclick = goToCustomerHome; $('#login-form').onsubmit = login; $('#register-form').onsubmit = register; document.querySelectorAll('[data-auth-mode]').forEach(button => button.onclick = () => setAuthMode(button.dataset.authMode)); $('#logout').onclick = () => { localStorage.removeItem('foodiehub-token'); state.token = ''; ownerView(true, 'login'); }; $('#status-filter').onchange = renderOrders; $('#menu-toggle').onclick = () => $('.nav-links').classList.toggle('mobile-open'); $('#order-type').onchange = () => $('#table-field').style.display = $('#order-type').value === 'Dine In' ? 'grid' : 'none'; if (!location.hash || location.hash === '#home') { document.getElementById('home')?.scrollIntoView({ behavior: 'instant', block: 'start' }); } else if (location.hash === '#menu') { document.getElementById('menu')?.scrollIntoView({ behavior: 'instant', block: 'start' }); } });
document.addEventListener('click', event => { const id = event.target.id; if (id === 'login-link') ownerView(true, 'login'); if (id === 'register-link') ownerView(true, 'register'); if (id === 'dashboard-link') ownerView(true, 'login'); if (id === 'menu-nav') { scrollToSection('menu'); } if (id === 'track-nav') { ownerView(false); location.hash = 'track'; document.getElementById('track')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); } if (id === 'home') { scrollToSection('home'); } if (event.target.closest('#back-home')) { goToCustomerHome(); } });
...
document.addEventListener('DOMContentLoaded', () => { const addItemButton = $('#add-item'); if (addItemButton) addItemButton.onclick = openAddItemModal; });
